...
// tinybf/webui/static/src/store/session.ts
var DEFAULT_CODE = `++++++++++[>+++++++>++++++++++>+++>+<<<<-]>++.>+.+++++++..+++.>++.<<+++++++++++++++.>.+++.------.--------.>+.>.`;
function cloneState(value) {
  return JSON.parse(JSON.stringify(value));
}
var useSessionStore = defineStore("session", () => {
  const codeInput = ref(DEFAULT_CODE);
  const inputValue = ref("");
  const language = ref("brainfuck");
  const tapeWindow = ref(10);
  ...
  const totalStepsCapped = ref(false);
  const selectedHistoryIndex = ref(0);
  const historyPreview = ref(false);
  const statusMessage = ref("Ready.");
  const statusIsError = ref(false);
  const isBusy = ref(false);
  const showCompiled = computed2(() => language.value === "tinybf");
  const breakpointSet = computed2(() => new Set(breakpoints.value));
  const historyRangeMax = computed2(() => sessionHistory.value.length > 0 ? sessionHistory.value.length - 1 : 0);
  const historyRangeDisabled = computed2(() => sessionHistory.value.length <= 1);
  const currentState = computed2(() => {
    var _a;
    return (_a = sessionHistory.value[selectedHistoryIndex.value]) != null ? _a : null;
  });
  const finishedAtCurrent = computed2(() => selectedHistoryIndex.value === sessionHistory.value.length - 1 && sessionFinished.value);
  const totalStepsDisplay = computed2(() => formatTotalSteps(totalSteps.value, totalStepsCapped.value, finishedAtCurrent.value));
  const historyInfoText = computed2(() => {
  ...
  const breakpointList = computed2(() => breakpoints.value.slice());
  const tapeCells = computed2(() => {
    const state = currentState.value;
    if (!state) {
      return [];
    }
  ...
  const codeWindowHtml = computed2(() => {
  ...
  const finishedDisplay = computed2(() => finishedAtCurrent.value ? "Yes" : "No");
  const commandDisplay = computed2(() => {
    const state = currentState.value;
    if (!state) {
      return "(init)";
    }
    if (state.command === null || state.command === void 0) {
      return "(init)";
    }
  ...
  function getSessionPayload() {
    ...
    const tapeWindowElement = document.getElementById("tape-window");
    if (codeElement) {
  ...
  function applySessionPayload(payload, selectLatestHistory) {
    sessionId.value = payload.session_id;
    sessionFinished.value = Boolean(payload.finished);
    brainfuckCode.value = payload.code || "";
    const nextHistory = Array.isArray(payload.history) && payload.history.length > 0 ? payload.history.map(cloneState) : "state" in payload && payload.state ? [cloneState(payload.state)] : [];
  ...
  function ensureScrollToCurrent() {
    nextTick(() => {
      const codeWindow = document.getElementById("code-window");
      const currentToken = codeWindow == null ? void 0 : codeWindow.querySelector(".code-token.is-current");
  ...
  async function createSession2() {
    ...
    try {
      const response = await createSession(getSessionPayload());
      if (!response.ok) {
  ...
  async function stepSession2() {
    ...
    try {
      const response = await stepSession(sessionId.value, { count });
  ...
  async function runSessionToEnd() {
    ...
    try {
      const response = await runSession(sessionId.value, { limit: DEFAULT_RUN_LIMIT, ignore_breakpoints: true });
      if (response.status === 409) {
        const detail = await response.json();
        setStatus(detail.detail || "Step limit exceeded.", true);
        return;
  ...
  async function runToBreakpoint() {
    ...
    try {
      ...
      const response = await runSession(sessionId.value, body);
      if (response.status === 409) {
        const detail = await response.json();
        setStatus(detail.detail || "Step limit exceeded.", true);
        return;
  ...
  async function resetSession2() {
    ...
    try {
      const response = await resetSession(sessionId.value);
      if (!response.ok) {
        throw new Error(await response.text());
      }
      const payload = await response.json();
  ...
  async function addBreakpoint2(pc) {
    if (!sessionId.value) {
    ...
    try {
      const body = { pc };
      const response = await addBreakpoint(sessionId.value, body);
      if (!response.ok) {
        throw new Error(await response.text());
      }
      const payload = await response.json();
  ...
  async function removeBreakpoint(pc) {
    ...
    setBusy(true);
    try {
      const response = await removeBreakpointRequest(sessionId.value, pc);
      if (!response.ok) {
        throw new Error(await response.text());
      }
      const payload = await response.json();
  ...
  async function toggleBreakpoint(pc) {
    ...
    if (breakpointSet.value.has(pc)) {
      ...
      await addBreakpoint2(pc);
    }
  }
  function selectHistory(index, preview) {
    if (!sessionHistory.value.length) {
      return;
    }
  ...
  function onHistorySliderInput(value) {
    selectHistory(value, true);
  }
  function onHistorySliderChange(value) {
    selectHistory(value, false);
  }
  function selectHistoryChip(index) {
  ...
  function setTapeWindowValue(value) {
    tapeWindow.value = Number.isFinite(value) ? value : tapeWindow.value;
  }
  function setStepCountValue(value) {
...
// tinybf/webui/static/src/app.ts
var App = defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      void session2.createSession();
    });
    watch(
      () => session2.codeWindowHtml,
      () => {
        session2.ensureScrollToCurrent();
      }
    );
    watch(
      () => session2.selectedHistoryIndex,
      () => {
        session2.ensureScrollToCurrent();
      }
    );
    function onHistoryInput(event) {
      ...
      session2.onHistorySliderInput(value);
    }
    function onHistoryChange(event) {
      const value = Number(event.target.value);
      session2.onHistorySliderChange(value);
    }
    function onLanguageChange(event) {
      session2.setLanguage(event.target.value);
    }
    function onCodeInput(event) {
      session2.setCode(event.target.value);
    }
    function onUserInput(event) {
      session2.setInputValue(event.target.value);
    }
    function onTapeWindowInput(event) {
      session2.setTapeWindowValue(Number(event.target.value));
    }
    function onStepCountInput(event) {
      session2.setStepCountValue(Number(event.target.value));
    }
    function onCodeWindowClick(event) {
      var _a;
      ...
      void session2.toggleBreakpoint(pc);
    }
    return {
      session: session2,
      historySlider,
        ...
        <div id="breakpoint-list" class="breakpoint-list">
          <template v-if="session.breakpointList.length">
            <span v-for="pc in session.breakpointList" :key="pc" class="breakpoint-chip">
              <span>pc {{ pc }}</span>
              <button type="button" :aria-label="'Remove breakpoint at pc ' + pc" @click.prevent="session.removeBreakpoint(pc)">\xD7</button>
            </span>
          </template>
          <template v-else>
...
// tinybf/webui/static/src/main.ts
